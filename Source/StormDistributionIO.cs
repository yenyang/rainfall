    ...
    class StormDistributionIO
    {
        ...
        public static bool reviewDepthCurve(SortedList<float, float> depthCurve, float stormDuration)
        {
            ...
            try {
                ...
                {
                    float lastValue=0f;
                    for (float f = 0; f < stormDuration; f += 3f)
                    {
        ...
        {
            float stormTimeStep = 3f;
            int maxStormDuration = 1440;
            if (stormDuration % stormTimeStep > 0f)
            {
                return false;
            }
            ...
            if (stormDuration <= maxStormDuration) {
                ...
                while (currentDepthVariance >= maxDepthVariance && currentTime <= maxStormDuration)
                {
                    maxDepthVariance = currentDepthVariance;
                    currentTime += stormTimeStep;
                    currentDepthVariance = oldDepthCurve[currentTime] - oldDepthCurve[currentTime-stormDuration];
                }
                if (currentTime <= maxStormDuration)
                {
                    float startTime = currentTime - stormTimeStep - stormDuration;
                    float startDepth = oldDepthCurve[startTime];
                    for (float f=startTime; f<=startTime+stormDuration; f += stormTimeStep)
                    {
