...
namespace Rainfall
{
	public class DrainageArea
	{
		...
		private int[] buildingGridIDs = new int[4];
		private Vector2[] buildingGridPositions = new Vector2[4];
		...
		public DrainageArea(int newID, Vector3 newPosition)
		{
			...
			buildingGridPositions[0] = new Vector2(m_position.x - gridSize / 4f, m_position.z - gridSize / 4f);
			buildingGridPositions[1] = new Vector2(m_position.x + gridSize / 4f, m_position.z - gridSize / 4f);
			buildingGridPositions[2] = new Vector2(m_position.x + gridSize / 4f, m_position.z + gridSize / 4f);
			buildingGridPositions[3] = new Vector2(m_position.x - gridSize / 4f, m_position.z + gridSize / 4f);
			int count = 0;
			foreach(Vector2 v in buildingGridPositions)
            {
				int buildingGridI = CheckBuildingGridZ(v);
				int buildingGridJ = CheckBuildingGridX(v);
				buildingGridIDs[count] = buildingGridI * buildingGridCoefficient + buildingGridJ;
				count++;
            }
		...
		private subbasin calculateBuildingImperviousArea(bool logging, bool calculatePOC)
        {
			...
			foreach (int buildingGridID in buildingGridIDs)
			{
				int iterator = 0;
				ushort buildingGridZX = _buildingManager.m_buildingGrid[buildingGridID];
		...
		private subbasin calculateSegmentImperviousArea(bool logging)
        {
			...
			float totalArea = 0f;
			float cummulativeImperviousArea = 0f;
			foreach (int segmentGridID in buildingGridIDs)
			{
				ushort segmentGridZX = _netManager.m_segmentGrid[segmentGridID];
				int iterator = 0;
